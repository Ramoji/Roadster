...
class PinnedFrequentStaticDetailTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = frequent.mileMaker
    ...
    func setUpViewForFavorite(){
        ...
        prepIconViews()
        if let frequent = frequent, let fullStateName = fullStateName{
            titleLabel.text = "Rest Stop on \((frequent.routeName as NSString)) in \(fullStateName.capitalized)"
            mapView.addAnnotation(frequent)
            latitudeLabel.text =  frequent.latitude.convertToString()
            longitudeLabel.text = frequent.longitude.convertToString()
